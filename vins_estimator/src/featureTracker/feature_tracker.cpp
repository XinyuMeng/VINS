...
void FeatureTracker::setMask()
{
    ...
    //上段代码是产生一个row行、col列、数据格式为Unsigned 8bits、数据值均为255的矩阵；
    // CV_8UC1中1代表一个通道、8U代表数据格式为Unsigned 8bits
...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    */
    ...
    if (1)
    {
        ...
        int n_max_cnt = MAX_CNT - static_cast<int>(cur_pts.size()); //需要检测的特征点个数
        if (n_max_cnt > 0)
        {
            if(mask.empty())
